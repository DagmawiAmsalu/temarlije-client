...
const CourseEdit = () => {
  ...
  const handleDrop = async ( e, index) => {
    ...
    let movingItem = allLessons[movingItemIndex]; // clicked/draged item to re-order
  ...
  const handleVideo = async (e) => {
    ...
    if(current.video && current.video.Location) {
      const res = await axios.post(
        `/api/course/video-remove/${values.teacher._id}`,
        current.video
        );
    ...
    const {data} = await axios.post(
      `/api/course/video-upload/${values.teacher._id}`,
      videoData,
    {
      onUploadProgress: (e) => 
  ...
  return (
    <TeacherRoute>
      ...
      <div className="pt-3 pb-3 toppad">
        <CourseCreateForm
